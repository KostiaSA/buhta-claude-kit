...
legacy-системой учёта. Покрывает **именованные запросы** (SchemaView), **пользовательские формы**
(движок dream), **печатные шаблоны отчётов** (FastReport) и **генерацию проводок из документов**,
плюс инструменты работы с БД MSSQL и HTTP-сервером автоматизации `.exe`.
...
- [skills/](skills/) — скилы плагина: `buhta-queries` (именованные запросы SchemaView),
  `buhta-forms` (формы dream), `buhta-reports` (печатные шаблоны),
  `buhta-genprovodka` (генерация проводок из документов).
- [docs/](docs/) — документация: схема данных и запросы, формы dream, руководства по формам и отчётам,
  генерация проводок ([docs/11](docs/11-генерация-проводок.md)).
